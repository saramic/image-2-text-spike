...
feature 'Michael plays a game of images 2 text', js: true do
  context 'spooky terms are set to play with' do
    before do
      %w[ghost pumpkin].each{|term| Term.create(term: term) }
    end
    ...
    scenario 'Michael plays a perfect game' do
      ...
      When 'Michael enters the winning term "ghost"' do
        focus_on(:round).enter_guess('ghost')
      end
